...
class buda (Exchange):
    ...
    def fetch_deposit_address(self, code, params={}):
        ...
        if self.is_fiat(currency):
            raise NotSupported(self.name + ': fiat fetchDepositAddress() for ' + currency + ' is not supported')
        ...
        if len(addressPool) == 0:
            raise AddressPending(self.name + ': there are no addresses ready for receiving ' + currency + ', retry again later)')
